...
public class Image extends CS355Image {
	...
	public void sharpen() {
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				...
				newPixels[width * y + x][0] = Math.max(Math.min(rgb[0], 255),0);
				newPixels[width * y + x][1] = Math.max(Math.min(rgb[1], 255),0);
				newPixels[width * y + x][2] = Math.max(Math.min(rgb[2], 255),0);
			}
		}
	...
	public void medianBlur() {
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				...
				// median color
				newPixels[width * y + x][0] = red[4];
				newPixels[width * y + x][1] = green[4];
				newPixels[width * y + x][2] = blue[4];
			}
		}
		updatePixels();
		bi=null;
	...
	public void contrast(int amount) {
		...
		for (int y = 0; y < super.getHeight(); ++y) {
			for (int x = 0; x < super.getWidth(); ++x) {
				rgb = super.getPixel(x, y, rgb);
				hsb = Color.RGBtoHSB(rgb[0], rgb[1], rgb[2], hsb);
				hsb[2] = scalar*(hsb[2]-0.5f)+0.5f;
				hsb[2] = Math.max(Math.min(hsb[2], 1.0f), 0.0f);
				Color c = Color.getHSBColor(hsb[0], hsb[1], hsb[2]);
				rgb[0] = c.getRed();
				rgb[1] = c.getGreen();
				rgb[2] = c.getBlue();
	...
	private void setNewPixels() {
		if(newPixels == null) {
			newPixels = new int[super.getWidth() * super.getHeight()][3];
		}
	}
	...
	private void updatePixels() {
		...
		for (int y = 0; y < height; ++y) {
			for (int x = 0; x < width; ++x) {
				setPixel(x, y, newPixels[width * y + x]);
			}
		}
	}
