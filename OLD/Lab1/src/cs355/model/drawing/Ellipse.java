...
public class Ellipse extends Shape {
	...
	public boolean pointInShape(Point2D.Double pt, double tolerance) {
		...
		//if(Math.abs(objCoord.distance(new Point2D.Double(0, 0))) <= radius) {
		//	return true;
		//}
		
		double a = Math.pow(objCoord.getX(), 2) / Math.pow(width, 2);
		double b = Math.pow(objCoord.getY(), 2) / Math.pow(height, 2);
